...
class EditJob extends Component {
    ...
    updateAction = async (props) => {
        ...
        if (isUpdate) {
            let res = await JobApi.getOne({ id: props.match.params.id, session: this.props.session });
            let job = res.body.data;
            job.status = job.status.id;
            if (job) {
                this.state.editor.setData(job.content);
                this.setState({
                    job,
                    statusSelectedOption: this.state.statusOptions.filter(el => el.value === job.status),
                });
            }
        } else {
            this.renewForm();
    ...
    handleSave = (e) => {
        ...
        let { job } = this.state;
        job.content = this.state.editor.getData();
        if (job.id) {
            this.props.updateJob(job).then(code => {
                if (code === 200) {
            ...
            this.props.addJob(job).then(code => {
                if (code === 200) {
    ...
    handleChangeStatus = (statusSelectedOption) => {
        ...
        let { job } = this.state;
        job.status = statusSelectedOption.value
        this.setState({
            job
        });
    }
                        ...
                        <div className="col-xs-12">
                            <div className="box box-primary">
                                <div className="box-header">
                                    <h3 className="box-title">{(this.state.isUpdate ? 'Cập nhật' : 'Thêm') + ' nghề'}</h3>
                                </div>
                                {/* <!-- /.box-header --> */}
...
const mapDispatchToProps = (dispatch, props) => {
    return {
        addJob: job => dispatch(actions.addJobApi(job)),
        updateJob: job => dispatch(actions.updateJobApi(job)),
